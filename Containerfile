...
# FIX BEDROCK (Booting Crash Fix): La generazione dell'initramfs e la rimozione
# dei kernel vecchi sono relegate esclusivamente al Layer 0 (ermete-base-nvidia).
# Eseguire dracut qui nel Layer 1 (senza --add ostree e senza i driver KMS nvidia)
# sovrascriveva l'initramfs perfetto del Layer 0, rendendo impossibile il boot dopo GRUB.
