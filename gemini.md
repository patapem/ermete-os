...
1. **Sincronia Niri-Systemd**: Nessuna utility di sessione o daemon background (`swaybg`, `ironbar`, `polkit-gnome.service`) si impone al Wayland Compositor. Al contrario Niri agisce nativamente come portale verso DBus, esportando le proprie coordinate d'ambiente `WAYLAND_DISPLAY`, e lancia la `niri-session.target`. Da quel millisecondo, Systemd-User domina tutto.
