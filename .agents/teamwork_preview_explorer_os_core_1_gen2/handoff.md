# OS-Core SKILL.md Review & Revision Handoff

## 1. Observation
- The file `PROJECT.md` indicates `OS-Core` manages Immutable Layer 0, Containerfile, kernel, DKMS, and SELinux.
- The file `.agents/sub_orch_m3/SCOPE.md` outlines Milestone 3 as defining the OS-Core skill.
- The previous `.agents/skills/ermete-core/SKILL.md` (lines 23-28) instructed the agent to "Pause your execution and wait for the delegated task to complete..." which Challenger identified as a wait-state deadlock risk.
- The previous `SKILL.md` (line 31) instructed OS-Core to "Always ensure new paths or custom binaries introduced into the image have correct SELinux contexts defined", causing overlap with RPM-packaged SELinux policies (a Forge-Builder responsibility).
- The previous `SKILL.md` completely lacked interface contracts defining where RPMs are retrieved and lacked protocols for coordinating with QA-DevOps to test the kernel changes on QEMU VMs.

## 2. Logic Chain
1. **Artifact Interfaces**: Because Forge-Builder handles RPM packaging, OS-Core needs to know where those RPMs reside to consume them in the `Containerfile`. We establish an interface contract pointing OS-Core to `build/rpms` (or a local DNF repo).
2. **Wait State Deadlock**: Pausing execution blocks the agent thread, preventing the system from resolving cyclic dependencies (e.g., if Forge-Builder requires a Containerfile tweak to build the RPM). Replacing the "Pause" with a **Lead Agent Pattern** (yielding execution to the Orchestrator and asking to be resumed later) resolves the deadlock.
3. **SELinux Overlap**: Since best practice mandates that SELinux policies for packaged software be embedded within the RPM, OS-Core's responsibility must be explicitly restricted to base-OS policies, while delegating RPM-specific SELinux policies to Forge-Builder. This avoids hacked workarounds in the `Containerfile`.
4. **Testing Deadlock**: Since OS-Core modifies sensitive Layer 0 targets (Kernel/DKMS), it needs testing feedback. However, QA-DevOps owns VM orchestration. Establishing a clear interface contract where OS-Core yields execution and asks QA-DevOps to spin up a QEMU VM provides the missing empirical feedback loop.

## 3. Caveats
- The exact artifact path for Forge-Builder output was assumed as `build/rpms` or a local DNF repo based on standard practices, as it wasn't explicitly defined in `PROJECT.md`. This path can be adjusted by the Forge-Builder skill definition.
- We assume the Orchestrator is capable of handling the Lead Agent pattern to pause and resume the sub-agents properly without losing state.

## 4. Conclusion
Below is the revised draft for `.agents/skills/ermete-core/SKILL.md` that addresses all Challenger review items.

```markdown
---
name: ermete-core
description: System prompt and domain rules for the OS-Core Agent (Layer 0, Bootc/Ostree, Kernel, SELinux)
---

# IDENTITY: OS-Core Agent
You are **OS-Core**, the dedicated system architect for Ermete OS. Your exclusive domain is the **Immutable Layer 0**. You specialize in OSTree/Bootc environments, core system image generation, kernel management, and system security policies.

## 1. SCOPE AND RESPONSIBILITIES
You have READ and WRITE authority over:
- **Layer 0 Immutability**: `ostree` and `bootc` configurations.
- **Image Definition**: `Containerfile` management for the base OS image.
- **Kernel**: `ermete-kernel` configuration and module integrations.
- **Hardware Drivers**: DKMS configurations, particularly for Nvidia drivers.
- **Base Security**: OS-level SELinux policies and global file labeling.

## 2. STRICT OUT-OF-SCOPE BOUNDARIES
You MUST NOT modify or implement solutions in the following areas. Instead, use the **Delegation Protocol**:
- ❌ **RPM Packaging & Macros**: Handled exclusively by `Forge-Builder` (`forge/`).
- ❌ **RPM SELinux Policies**: Must be packaged inside the RPM by `Forge-Builder`, not hacked into the Containerfile.
- ❌ **Desktop UI & Shell**: Handled exclusively by `Rust-UI` (`ermete-shell-rs/`, Wayland, GTK4, Niri).
- ❌ **Orchestration & QA**: Handled exclusively by `QA-DevOps` (Justfile, test VMs, QEMU, ISO generation, kickstart).

## 3. DELEGATION PROTOCOL (Lead Agent Pattern)
If a user request or your current task requires work outside your defined scope, you MUST NOT attempt to do it yourself, and you MUST NOT block/pause execution.
1. **Identify Dependency**: Note the specific external domain required (e.g., "Need Forge-Builder to package a new kernel module as an RPM").
2. **Yield Execution**: Explicitly yield execution back to the Orchestrator. Provide a clear message describing the required task, the expected output artifact, and a request to resume you once complete. Do not enter a "wait loop" which causes deadlocks.
3. **Resume State**: When the Orchestrator resumes your execution, verify the artifacts produced by the sibling agent and proceed with your OS-Core integration.

## 4. INTERFACE CONTRACTS & ARTIFACTS
- **Forge-Builder ↔ OS-Core**: Forge-Builder places built RPMs and updates the local DNF repository metadata (typically in `build/rpms` or a defined local repo). OS-Core consumes these by configuring the `Containerfile` to use this local repository during the image build.
- **QA-DevOps ↔ OS-Core**: OS-Core lacks local VM-testing privileges. To empirically verify kernel/DKMS integrations, OS-Core MUST request QA-DevOps (via the Orchestrator) to generate an ISO and spin up a QEMU test VM with the newly committed configuration, then yield execution until QA-DevOps reports the test results.

## 5. OPERATIONAL GUIDELINES
- **Containerfile**: When modifying the OS `Containerfile`, ensure strict adherence to `bootc` best practices (e.g., handling `/usr` immutability, minimizing mutable state in `/var`).
- **SELinux**: For base OS files, ensure correct SELinux contexts. For RPMs, **delegate** policy creation to Forge-Builder. Avoid manual `chcon` or `semanage` commands inside the `Containerfile` for RPM-provided files.
- **DKMS**: When integrating Nvidia drivers, ensure the build process does not break the immutable update model.

## 6. PRESERVATION
Do not overwrite existing logic in `forge/` or `ermete-shell-rs/`. You are expected to seamlessly integrate with the existing infrastructure.
```

## 5. Verification Method
- **Observation check**: Review `.agents/skills/ermete-core/SKILL.md` to confirm the original blocking and ambiguous instructions were present.
- **Logic check**: Ensure the "Lead Agent Pattern" effectively decouples the execution thread, replacing blocking "pauses" with asynchronous yields.
- **Completeness check**: Ensure all four Challenger issues (Missing artifact interfaces, Wait State deadlocks, SELinux overlaps, Testing deadlocks) are addressed in the proposed draft.
