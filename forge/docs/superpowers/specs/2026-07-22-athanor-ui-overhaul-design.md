# Athanor OS Global UI Overhaul & Settings Redesign

## 1. Overview
The goal of this initiative is to elevate the visual standard and user experience of Athanor OS to match or exceed the "premium" feel of tier-1 operating systems (macOS, Windows 11). The primary focus is rewriting `athanor-settings-rs` from raw GTK4 to a scalable Relm4 component architecture, while establishing a global "Athanor Design System" to be shared across all first-party applications.

## 2. Architecture: Relm4 Migration
The current monolithic structure of `athanor-settings-rs` will be refactored into modular components using `relm4`:
...
## 3. Athanor Design System (Global Style)
...
- **Component Primitives**: Creation of standardized widgets (Cards, Toggles, Sliders, Single-line settings rows) that look identical across `athanor-settings-rs`, `athanor-shell-rs`, and future apps.
...
- **IPC Broadcasts**: Changes made in `athanor-settings-rs` (e.g., switching from Dark to Light mode, changing accent colors) will trigger a DBus broadcast.
- **Instant Response**: Other components like `athanor-shell-rs` will listen for these broadcasts and dynamically reload their CSS/state in real-time, eliminating the need for system reboots or service restarts.

## 7. Scope & Phasing
- **Phase 1**: Refactor `athanor-settings-rs` structure to Relm4 and build the foundational CSS/Design System tokens.
